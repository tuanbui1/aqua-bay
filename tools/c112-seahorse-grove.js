// C112 — after a C111 DIVE FOR SEAHORSE board tap the C86 dash
// used to land in 6m Shallows with ribbon “Point the glowing
// cone at the SHINY clownfish”. Seahorses live in Seahorse
// groves (zoneBandForSpecies(5), y≥OCEAN_BASE_H). C112 arms a
// dive-for hunt on goto-dive-for so that dive opens the species
// band. A normal DIVE chip (no hunt) still starts in shallows
// so new-game first-clownfish / TODAY 1 stays. Walk still does
// not buy. Phone 390×844 first. Isolated SAVE_KEY. Turtle
// unlocked, Seahorse locked, $4000. Continue.
const fs = require("fs");
...
const names = [
  ...
  "oceanEntrySpawn", "diveForHuntGoal", "nearestHuntFish", "plazaWalkChipLegal",
...
// 1) Stamp loop 112 is asserted above.
...
// 3) A fresh new-game / first DIVE (no dive-for) still hunts clownfish in shallows.
...
// 4) Walk still does not auto-buy. Desktop hold-W still no auto-buy.
...
console.log("c112 seahorse grove: ok (next=" + openApi.nextLockedTank() +
