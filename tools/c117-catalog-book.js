// C117 — C108 only flips the gold HUD catalog chip to
// BOOK while plazaWalkChipLegal() (dock + wood ↑ SHOP).
// At TAP TO UNLOCK / DIVE FOR bowls and in the ocean
// hunt the same chip reverts to SHOP and sits next to
// pause on a 390 thumb. Walk-north already owns SHOP.
// Catalog never says SHOP — BOOK on the dock, at the
// bowls, and in the 70m hunt. CLOSE while the tray is
// open stays. C116 TODAY=Stock Seahorse on the hunt
// stays. C115 thumb DIVE still arms the hunt. C114
// hide-SURFACE stays. Walk still does not buy. Do not
// auto-dive / buy / stock. Phone 390×844 first.
// Isolated SAVE_KEY. Turtle unlocked, Seahorse locked
// then unlocked via TAP TO UNLOCK, $4000. Continue.
const fs = require("fs");
...
assert(/Aqua Bay · loop 117/.test(src), "title/pause stamp is loop 117");
...
// 1) Stamp loop 117 is asserted above.
// 2) Seed Turtle open / Seahorse locked / $4000. Catalog BOOK
//    at TAP TO UNLOCK / DIVE FOR, on the 70m hunt, and on the
//    dock with wood ↑ SHOP. CLOSE while the tray is open.
//    TODAY still Stock Seahorse on the hunt (C116).
...
console.log("c117 catalog book: ok (next=" + openApi.nextLockedTank() +
