// C118 — C116 only promotes TODAY once diveForHunt is
// armed, so the empty Seahorse bowl never reads Stock
// Seahorse until the player is already at 70m groves.
// After TAP TO UNLOCK an empty unlocked bowl (DIVE FOR
// board legal, bag empty of that fish), plaza TODAY is
// visible and reads Stock Seahorse. Unlock toast /
// ribbon must not eat TODAY. Do not wait until the
// ocean hunt is armed. Catalog stays BOOK. C116 ocean
...
assert(/Aqua Bay · loop 118/.test(src), "title/pause stamp is loop 118");
...
// 1) Stamp loop 118 is asserted above.
...
//    Thumb DIVE still 70m groves + C116 ocean TODAY.
...
console.log("c118 plaza today: ok (next=" + openApi.nextLockedTank() +
