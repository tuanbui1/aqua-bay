// C115 — after TAP TO UNLOCK the DIVE FOR SEAHORSE board
// sits on the empty bowl and the thumb DIVE chip stays
// live. Loop 114 hid SURFACE on that hunt but left this
// easier DIVE dropping a shallows first-dive (C98 SURFACE
// + shiny clown). While the DIVE FOR board is legal (just
// unlocked empty bowl, bag empty of that fish), thumb DIVE
// arms the same diveForHunt and drops in Seahorse groves.
// Copy stays DIVE. Do not hide DIVE. Do not auto-dive on
// unlock. Before unlock / after the hunt is done / bowl
// stocked: regular shallows. New-game first DIVE still
// shallows / shiny clownfish. C114 hide-SURFACE stays.
// Walk still does not buy. Phone 390×844 first. Isolated
// SAVE_KEY. Turtle unlocked, Seahorse locked then
// unlocked via TAP TO UNLOCK, $4000. Continue.
const fs = require("fs");
...
assert(/Aqua Bay · loop 115/.test(src), "title/pause stamp is loop 115");
...
// 1) Stamp loop 115 is asserted above.
// 2) Unlock Seahorse. DIVE FOR SEAHORSE on bowl. Thumb DIVE
//    still visible. Tap thumb DIVE (not the bowl board).
...
console.log("c115 dive chip arms the hunt: ok (next=" + openApi.nextLockedTank() +
  ...
  ", freshY=" + freshSpawn.y +
  ", freshSurf=" + fresh.surfaceAssistLegal() +
