// C122 — after ↑ SURFACE, beginSurface fades ocean →
// shop (fadeSpeed 7.2) and pinSurfaceDockCam lands
// them on the dock. leadStockAfterSurface used to
// setWalkDest to the glowing tank and camEase-taxi
// north to the bowls. On a 390 portrait that reads
// as a hard cut from the bay into the fish-tank
// room, not "I came up the pier." After SURFACE they
// land on the dock (bay / DIVE pad, y~1000) and stay
// a beat. Do not auto-walk to the tank. Do not
// auto-stock. TODAY still Stock Seahorse. C100
// tap-to-stock board / ribbon can cue the walk —
// the player taps it (or walks) themselves. Empty
// bag SURFACE still docks. C121 SURFACE-vs-ribbon
// stays. Walk still does not buy. Do not auto-dive
// / buy / stock / surface. Do not camera-clamp.
// Phone 390×844 first. Isolated SAVE_KEY. Turtle
// unlocked, Seahorse locked then unlocked via TAP
// TO UNLOCK, $4000. Continue.
const fs = require("fs");
...
assert(/Aqua Bay · loop 122/.test(src), "title/pause stamp is loop 122");
...
console.log("c122 surface dock: ok (next=" + openApi.nextLockedTank() +
