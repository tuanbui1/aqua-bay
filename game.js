...
(() => {
  ...
  function shopWalkRects() {
    ...
    // then pinned a dock walker at y≈788. Walk the south aprons plus a
    // west spine so the aisle routes around the bowls.
  ...
  function drawTitle() {
    ...
    ctx.fillText("Aqua Bay · loop 101", W / 2, lay.stampY);
    ctx.restore();
  ...
  function drawPause() {
    ...
    if (state.mode === "help") {
      ...
      ctx.fillText("Aqua Bay · loop 101", W / 2, footY + (tall ? 28 : 20));
      ...
      ctx.fillText("Aqua Bay · loop 101", W / 2, footY + (tall ? 52 : 36));
    }
    ctx.restore();
    menuYShift = 0;
  }
