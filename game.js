...
(() => {
  ...
  function drawPierBoards(x, y, w, h, opts) {
    ...
    for (let row = 0, yy = y; yy < y + h; row++, yy += plank) {
      ...
      if (ph <= 0) break;
      const lit = sunAmt(x + w * 0.55, yy);
      const base = teal ? (row % 2 ? 200 : 216) : (row % 2 ? 194 : 216);
      const r = (base * (0.7 + lit * 0.4)) | 0;
      const g = ((teal ? base - 6 : base - 30) * (0.68 + lit * 0.38)) | 0;
      const b = ((teal ? base - 22 : base - 74) * (0.66 + lit * 0.32)) | 0;
      ctx.fillStyle = "rgb(" + r + "," + g + "," + b + ")";
      ctx.fillRect(x, yy, w, ph);
      ctx.strokeStyle = "rgba(90,48,16," + (0.09 + (1 - lit) * 0.08) + ")";
      ctx.lineWidth = 1;
      for (let k = 1; k < 3; k++) {
        const gy = yy + ph * (0.3 * k);
        ctx.beginPath();
        ctx.moveTo(x + 4, gy);
        ctx.bezierCurveTo(x + w * 0.34, gy + 1.3, x + w * 0.66, gy - 1.2, x + w - 4, gy);
        ctx.stroke();
      }
      ctx.fillStyle = "rgba(58,30,10,0.48)";
      ctx.fillRect(x, yy + ph, w, 2);
      ctx.fillStyle = "rgba(36,24,14,0.58)";
      ctx.beginPath(); ctx.arc(x + 8, yy + 5, 1.25, 0, Math.PI * 2); ctx.fill();
      ctx.beginPath(); ctx.arc(x + w - 8, yy + 5, 1.25, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = "rgba(255,220,160,0.28)";
      ctx.beginPath(); ctx.arc(x + 7.4, yy + 4.4, 0.5, 0, Math.PI * 2); ctx.fill();
    }
    if (wetY != null) {
      const wet = ctx.createLinearGradient(x, wetY - 40, x, wetY + 10);
      wet.addColorStop(0, "rgba(30,90,110,0)");
      wet.addColorStop(0.5, "rgba(36,108,128,0.14)");
      wet.addColorStop(1, "rgba(18,64,82,0.4)");
      ctx.fillStyle = wet;
  ...
  function drawFishBody(sp, x, y, ang, scale, t) {
    ...
    if (sp.id === 0) {
      ...
      ctx.beginPath();
      ctx.moveTo(-2.2 * s, -6.8 * s);
      ctx.quadraticCurveTo(1.2 * s, -13.4 * s + wob * 2, 5.4 * s, -7.2 * s);
      ctx.lineTo(1.2 * s, -6.2 * s);
      ctx.closePath(); ctx.fill();
      ctx.strokeStyle = "#5a2a10"; ctx.lineWidth = 0.85; ctx.stroke();
      ctx.fillStyle = "#e07020";
      ctx.beginPath();
      ctx.moveTo(1.4 * s, 6.6 * s);
      ctx.quadraticCurveTo(4.2 * s, 11.2 * s - wob * 2, 7.2 * s, 6.4 * s);
      ctx.closePath(); ctx.fill();
      ctx.save();
      ctx.translate(2.4 * s, 2.6 * s);
      ctx.rotate(0.55 + wob * 0.4);
      ctx.fillStyle = "#f6a04a";
      ctx.beginPath(); ctx.ellipse(0, 0, 4.4 * s, 1.7 * s, 0, 0, Math.PI * 2); ctx.fill();
      ctx.restore();
      ...
      ctx.fillStyle = body;
      ctx.beginPath(); ctx.ellipse(0.6 * s, 0, 10.6 * s, 7.6 * s, 0, 0, Math.PI * 2); ctx.fill();
      ctx.save();
      ctx.beginPath(); ctx.ellipse(0.6 * s, 0, 10.6 * s, 7.6 * s, 0, 0, Math.PI * 2); ctx.clip();
      ctx.fillStyle = "#fff6e8";
      ctx.beginPath();
      ctx.moveTo(-5.6 * s, -8 * s);
      ctx.quadraticCurveTo(-2.4 * s, 0, -5.8 * s, 8 * s);
      ctx.lineTo(-1.6 * s, 8 * s);
      ctx.quadraticCurveTo(1.4 * s, 0, -1.4 * s, -8 * s);
      ctx.closePath(); ctx.fill();
      ctx.beginPath();
      ctx.moveTo(2.2 * s, -8 * s);
      ctx.quadraticCurveTo(4.6 * s, 0, 2.0 * s, 8 * s);
      ctx.lineTo(5.4 * s, 8 * s);
      ctx.quadraticCurveTo(7.6 * s, 0, 5.6 * s, -8 * s);
      ctx.closePath(); ctx.fill();
      ctx.strokeStyle = "#2a1508"; ctx.lineWidth = 1.15 * Math.max(0.7, s);
      ctx.beginPath();
      ctx.moveTo(-5.6 * s, -8 * s); ctx.quadraticCurveTo(-2.4 * s, 0, -5.8 * s, 8 * s);
      ctx.moveTo(-1.4 * s, -8 * s); ctx.quadraticCurveTo(1.4 * s, 0, -1.6 * s, 8 * s);
      ctx.moveTo(2.2 * s, -8 * s); ctx.quadraticCurveTo(4.6 * s, 0, 2.0 * s, 8 * s);
      ctx.moveTo(5.6 * s, -8 * s); ctx.quadraticCurveTo(7.6 * s, 0, 5.4 * s, 8 * s);
      ctx.stroke();
      ctx.restore();
      ctx.strokeStyle = "#5a2a10"; ctx.lineWidth = 1.45;
      ctx.beginPath(); ctx.ellipse(0.6 * s, 0, 10.6 * s, 7.6 * s, 0, 0, Math.PI * 2); ctx.stroke();
      drawFishEye(s, 6.6, -1.5, look.x, look.y);
    } else if (sp.id === 1) {
      ctx.fillStyle = "#ffe14a";
      ctx.beginPath();
      ctx.moveTo(-6.4 * s, 0);
      ctx.quadraticCurveTo(-12 * s, -12.6 * s + wob * 4, -20.6 * s, -9.2 * s + wob * 5);
      ctx.quadraticCurveTo(-14.4 * s, 0, -20.6 * s, 9.2 * s - wob * 5);
      ctx.quadraticCurveTo(-12 * s, 12.2 * s - wob * 4, -6.4 * s, 0);
      ctx.closePath(); ctx.fill();
      ctx.strokeStyle = "#c4a010"; ctx.lineWidth = 1; ctx.stroke();
      ctx.fillStyle = "#ffe14a";
      ctx.beginPath();
      ctx.moveTo(-2.4 * s, -9.2 * s);
      ctx.quadraticCurveTo(3.2 * s, -16.4 * s + wob * 3, 9.4 * s, -9.8 * s);
      ctx.lineTo(2.2 * s, -8.4 * s);
      ctx.closePath(); ctx.fill();
      ctx.beginPath();
      ctx.moveTo(-2.4 * s, 9.2 * s);
      ctx.quadraticCurveTo(3.2 * s, 15.8 * s - wob * 3, 8.8 * s, 9.6 * s);
      ctx.lineTo(2.2 * s, 8.2 * s);
      ctx.closePath(); ctx.fill();
  ...
  function drawPlayer(x, y, pose) {
    ...
    if (skin === "dino") {
      ...
      fillCapsule(-5.2, 8, -4.6, 18.4 + walk * 4.5, 2.5);
      fillCapsule(5.0, 8, 5.4, 18.4 - walk * 4.5, 2.5);
      ...
      ctx.save(); ctx.translate(-11.4, -1); ctx.rotate(0.22 + swing * 0.09);
      fillCapsule(0, 0, 0.4, 13, 2.2); ctx.restore();
      ctx.save(); ctx.translate(11.4, -1); ctx.rotate(-0.22 - swing * 0.09);
      fillCapsule(0, 0, -0.4, 13, 2.2); ctx.restore();
  ...
  function drawTitle() {
    ...
    drawBayWater(-20, H * 0.38, W + 40, H * 0.7, state.time, false);
    drawFoamBand(-10, H * 0.38 + 8, W + 20, state.time);
    drawPierBoards(-8, H * 0.72, W + 16, H * 0.32, { plank: 22, wetY: H * 0.74 });
    ctx.save(); ctx.globalCompositeOperation = "lighter";
    for (let i = 0; i < 5; i++) {
