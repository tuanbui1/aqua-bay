...
(() => {
  ...
  // Loop 48 characters/fish + loop 53 walk/swim + loop 55 skyline + loop 56 cone/props + loop 57 pier/paddle + loop 58 plant + loop 59 clean blit/pier + loop 60 plant props + loop 61 NPC plate/shadow + loop 62 continuous pier + loop 63 water-on-water + loop 64 seabed/DIVE.
  ...
  function drawHangingSign(x, y) {
    ...
    sitShadow(x + 3, y + 16, 18, 6.2, 0.44);
  ...
  function paintUniqueTown(g, w, h, base, sc) {
    const s = sc || 1;
    ...
    for (let i = 0; i < specs.length; i++) {
      ...
      if (b.kind === "tower") {
        ...
        g.fillRect(bx + bw * 0.38, by + bh * 0.58, bw * 0.24, bh * 0.42);
        g.fillStyle = "#c4483a";
        g.fillRect(bx + bw * 0.3, by + bh * 0.12, bw * 0.4, 7);
        g.fillStyle = "#fff6e8";
        g.font = "bold " + Math.max(7, (bw * 0.12) | 0) + "px sans-serif";
        g.textAlign = "center";
        g.fillText("OPEN", bx + bw * 0.5, by + bh * 0.12 + 6);
  ...
  function drawPierPost(x, footY, sc, seed) {
    ...
    const lean = (hash2(id, 1) - 0.5) * 0.22;
    const hMul = 0.62 + hash2(id, 2) * 0.72;
    const footW = (10 + hash2(id, 4) * 11) * s;
    const footH = (2.4 + hash2(id, 5) * 4.4) * s;
    const shear = Math.sin(t * 1.15 + id * 1.7) * (0.22 + hash2(id, 6) * 0.38);
    const bob = Math.sin(t * 1.55 + id * 0.85) * (2.2 + hash2(id, 7) * 3.4);
    const stain = hash2(id, 12);
    ctx.fillStyle = "rgba(8, 24, 32," + (0.16 + hash2(id, 8) * 0.2) + ")";
    ctx.beginPath();
    ctx.ellipse(x + shear * 14, footY + 6 + bob * 0.2, footW, footH, shear * 0.7, 0, Math.PI * 2);
    ctx.fill();
    ctx.save();
    ctx.translate(x + Math.sin(t * 1.4 + id) * (5 + hash2(id, 9) * 6), footY + 14 + bob);
    ctx.globalAlpha = 0.12 + hash2(id, 10) * 0.16;
    ctx.beginPath();
    const rw = 8 * s * hMul, rh = 18 * s * hMul * (0.28 + hash2(id, 11) * 0.22);
    for (let i = 0; i <= 8; i++) {
      const u = i / 8;
      const px = Math.sin(u * Math.PI * 2 + t * 2.1 + id) * rw * (0.7 + 0.3 * Math.sin(t + i));
      const py = u * rh * 2;
      if (i === 0) ctx.moveTo(px, py); else ctx.lineTo(px, py);
    }
    ctx.strokeStyle = "rgba(6, 18, 24, 0.55)";
    ctx.lineWidth = 7 * s * (0.7 + hash2(id, 19) * 0.6);
    ctx.stroke();
    ctx.restore();
    ctx.save();
    ...
    ctx.fillStyle = "rgba(255, 214, 150," + (0.06 + hash2(id, 21) * 0.1) + ")";
    ctx.fillRect(-8 * s, -42 * s, 5 * s, 42 * s);
    const ringKind = hash2(id, 15);
    if (ringKind > 0.62) {
      ctx.strokeStyle = "rgba(196, 96, 48, 0.8)";
      ctx.lineWidth = 2.4 + hash2(id, 16) * 2.2;
      ctx.beginPath();
      ctx.arc(0, -12 * s - hash2(id, 17) * 18 * s, 8 * s + hash2(id, 22) * 3, 0.2, 2.9);
      ctx.stroke();
    } else if (ringKind > 0.28) {
      ctx.strokeStyle = "rgba(160, 128, 64, 0.7)";
      ctx.lineWidth = 1.6 + hash2(id, 16);
      ctx.beginPath();
      ctx.arc(0, -20 * s - hash2(id, 17) * 10 * s, 7.2 * s, 0.4, 2.6);
      ctx.stroke();
    }
    ctx.fillStyle = "rgba(210, 200, 180, 0.7)";
    const barn = 1 + ((hash2(id, 18) * 5) | 0);
    for (let i = 0; i < barn; i++) {
      ctx.beginPath();
      ctx.arc((-5 + hash2(id, 20 + i) * 10) * s, (-6 - hash2(id, 30 + i) * 22) * s, 1.0 + hash2(id, 40 + i) * 1.4, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.restore();
  }
  ...
  function drawZoneBed(z) {
    ...
    for (let n = 0; n < nPatch; n++) {
      ...
      const py = y1 - 6 - hash2(seed, 40 + n) * 52;
      const rx = 64 + hash2(seed, 60 + n) * 168;
      const ry = 12 + hash2(seed, 80 + n) * 30;
      const warm = hash2(seed, 90 + n);
      ctx.fillStyle = warm > 0.62
        ? "rgba(236, 214, 148, 0.3)"
        : warm < 0.28
          ? "rgba(86, 112, 72, 0.24)"
          : "rgba(176, 140, 72, 0.2)";
      ctx.beginPath();
      ctx.ellipse(px, py, rx, ry, (warm - 0.5) * 0.7, 0, Math.PI * 2);
      ctx.fill();
    }
    ...
    if (z.forever) {
      ...
      if (kind === 0) {
        ...
        for (let i = 0; i < 18; i++) {
          const px = hash2(seed, 400 + i) * OCEAN.w;
          const py = y1 - 12 - hash2(seed, 420 + i) * 48;
          ctx.globalAlpha = 0.2 + 0.35 * (0.5 + 0.5 * Math.sin(state.time * 2 + i));
          ctx.beginPath(); ctx.arc(px, py, 1.6 + hash2(seed, 440 + i) * 1.8, 0, Math.PI * 2); ctx.fill();
        }
        ctx.globalAlpha = 1;
      } else if (kind === 1) {
        ctx.fillStyle = "rgba(200,240,255,0.28)";
        let px = 40 + hash2(seed, 9) * 80;
        let i = 0;
        while (px < OCEAN.w - 20) {
          const py = y1 - 6 - hash2(seed, 450 + i) * 18;
          plantInSand(px + 8, py + 4, 14 + hash2(seed, 460 + i) * 8, 6, hash2(seed, 470 + i));
          ctx.fillStyle = "rgba(200,240,255,0.32)";
          ctx.beginPath();
          ctx.moveTo(px, py);
          ctx.lineTo(px + 5 + hash2(seed, 480 + i) * 8, py - 28 - hash2(seed, 490 + i) * 16);
          ctx.lineTo(px + 14 + hash2(seed, 500 + i) * 6, py);
          ctx.fill();
          px += 70 + hash2(seed, 510 + i) * 140;
          i++;
        }
      } else if (kind === 2) {
      ...
      let px = 28 + hash2(seed, 5) * 50;
      let i = 0;
      while (px < OCEAN.w - 16) {
        const py = y1 - 8 - hash2(seed, 520 + i) * 16;
        plantInSand(px, py + 6, 16 + hash2(seed, 530 + i) * 10, 7, hash2(seed, 540 + i));
        ctx.strokeStyle = hash2(seed, 550 + i) > 0.5 ? "#2f8a5a" : "#3d8b4a";
        ctx.lineWidth = 1.8 + hash2(seed, 560 + i) * 1.6;
        const sway = Math.sin(state.time + i * 0.7) * (8 + hash2(seed, 570 + i) * 8);
        ctx.beginPath();
        ctx.moveTo(px, py + 4);
        ctx.quadraticCurveTo(px + sway * 0.45, py - 16, px + sway * 0.15, py - 32 - hash2(seed, 580 + i) * 18);
        ctx.stroke();
        px += 38 + hash2(seed, 590 + i) * 92;
        i++;
      }
    } else if (s === 1) {
      let px = 36 + hash2(seed, 6) * 60;
      let i = 0;
      while (px < OCEAN.w - 24) {
        const py = y1 - 4 - hash2(seed, 600 + i) * 14;
        plantInSand(px + 8, py + 4, 22 + hash2(seed, 610 + i) * 12, 8, hash2(seed, 620 + i));
        const rot = (hash2(seed, 630 + i) - 0.5) * 0.9;
        const sc = 0.7 + hash2(seed, 640 + i) * 0.7;
        ctx.fillStyle = hash2(seed, 650 + i) > 0.5 ? "#e85d6a" : "#c45ec8";
        ctx.beginPath();
        ctx.ellipse(px, py - 8, 14 * sc, 6 * sc, rot, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = "#5a6a70";
        ctx.beginPath();
        ctx.ellipse(px + 16 * sc, py + 1, 16 * sc, 8 * sc, rot * 0.4, 0, Math.PI * 2);
        ctx.fill();
        px += 70 + hash2(seed, 660 + i) * 130;
        i++;
      }
    } else if (s === 2) {
      ...
      let px = 50 + hash2(seed, 8) * 70;
      let i = 0;
      while (px < OCEAN.w - 20) {
        const py = y1 - 2 - hash2(seed, 670 + i) * 12;
        plantInSand(px, py + 4, 18 + hash2(seed, 680 + i) * 10, 7, hash2(seed, 690 + i));
        ctx.strokeStyle = s === 5 ? "#2a8a5a" : "rgba(120,200,220,0.4)";
        ctx.lineWidth = 2 + hash2(seed, 700 + i) * 1.4;
        const sway = Math.sin(state.time * 1.2 + i) * (10 + hash2(seed, 710 + i) * 8);
        ctx.beginPath();
        ctx.moveTo(px, py);
        ctx.quadraticCurveTo(px + sway * 0.5, py - 28, px + sway * 0.12, py - 52 - hash2(seed, 720 + i) * 24);
        ctx.stroke();
        px += 56 + hash2(seed, 730 + i) * 150;
        i++;
      }
    }
    ctx.restore();
  ...
  function drawPierBoards(x, y, w, h, opts) {
    ...
    for (let yy = startY; yy < y + h; yy += plank) {
      ...
      ctx.fillStyle = "rgba(24,12,6,0.14)";
      ctx.fillRect(rowX, yy + ph - 1, rowW, 1);
    }
    ctx.restore();
    ctx.save();
    if (taper) {
  ...
  function drawDeckPosts(x, y, w, h, n) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      drawPierPost(px, y + h + 6, 0.62 + hash2(id, 3) * 0.28, id);
    }
  }
  ...
  function drawDiver(x, y, ang, t, skinId) {
    ...
    const faceS = player.faceS != null ? player.faceS : (Math.cos(ang) < -0.1 ? -1 : 1);
    const flip = faceS < 0 ? -1 : 1;
    const headingPitch = Math.sin(ang) * 0.38;
    const pitch = clamp((player.pitch || 0) * 0.7 + headingPitch + kickWave * 0.05, -0.48, 0.48);
    ...
    const drew = blitGait(skin, "swim", fi, x, y, {
      rot: tilt * flip,
      scale: 0.58,
      scaleX: faceS * (1 + Math.abs(kickWave) * 0.04),
  ...
  function drawShop() {
    ...
    for (const [px, py, sc, id] of dockPosts) {
      if (py < 1008) drawPierPost(px, py, sc, id);
    }
    ...
    for (let i = 0; i < dockPosts.length; i++) {
      ...
      if (py < 1008) continue;
      actors.push({ y: py, draw: function () { drawPierPost(px, py, sc, id); } });
    }
    ...
    for (const f of list) {
      ...
      if (f.rare) {
        ...
        if (state.shinyCallout > 0) {
          ...
          drawWorldPlate(f.x, f.y - 56 + bounce, "SHINY!", "shiny");
          ctx.save();
  ...
  function drawDecorOcean() {
    ...
    // Unique mid-depth dunes — not 3 identical full-width sand rulers.
    ctx.save();
    for (const [yy, seed] of [[1240, 11], [1480, 23], [1720, 37]]) {
      for (let n = 0; n < 6; n++) {
        const px = 70 + hash2(seed, n) * (OCEAN.w - 140);
        const py = yy + (hash2(seed, n + 8) - 0.35) * 40;
        const rx = 80 + hash2(seed, n + 16) * 170;
        const ry = 12 + hash2(seed, n + 24) * 22;
        ctx.fillStyle = "rgba(176,148,78," + (0.09 + hash2(seed, n + 32) * 0.12) + ")";
        ctx.beginPath();
        ctx.ellipse(px, py, rx, ry, (hash2(seed, n + 40) - 0.5) * 0.45, 0, Math.PI * 2);
        ctx.fill();
      }
    }
  ...
  function drawSkinPicker(cx, cy, cardW, cardH, gap) {
    ...
    ctx.textAlign = "center";
    ctx.fillText("Who's diving?", cx, cy - 20);
    for (let i = 0; i < SKIN_IDS.length; i++) {
      ...
      if (id === "skip") {
        ...
        plate.addColorStop(0, "#f2c878");
        plate.addColorStop(0.42, "#d07850");
        plate.addColorStop(1, "#b07a42");
      }
      ctx.fillStyle = plate;
      ...
      if (id === "skip") {
        ...
        // Dusk harbor strip: right-side town + wide pier, not a teal slab.
        drawPierBoards(x + 3, cy + 8 + cardH * 0.44, cardW - 6, cardH * 0.22, {
          plank: 10,
        });
        ctx.save();
      ...
      drawPlayer(x + cardW / 2, cy + cardH * 0.62, {
        ...
        faceS: 1, drawScale: id === "dino" ? 1.82 : 1.65, paintOnly: id === "dino",
  ...
  function drawTitle() {
    ...
    card(W / 2 - 250, 40, 500, 156, "rgba(12, 28, 36, 0.78)");
    ctx.save();
    roundRect(W / 2 - 250, 40, 500, 156, 12); ctx.clip();
    ctx.save();
    roundRect(W / 2 - 210, 54, 420, 72, 10); ctx.clip();
    drawPierBoards(W / 2 - 210, 54, 420, 72, { plank: 16, seg: 78 });
    ctx.fillStyle = "rgba(40, 20, 8, 0.18)";
    ctx.fillRect(W / 2 - 210, 54, 420, 72);
    ctx.restore();
    ctx.strokeStyle = "#e8c04a"; ctx.lineWidth = 3;
    roundRect(W / 2 - 204, 59, 408, 62, 8); ctx.stroke();
    ctx.strokeStyle = "rgba(90, 48, 16, 0.55)"; ctx.lineWidth = 1.4;
    roundRect(W / 2 - 210, 54, 420, 72, 10); ctx.stroke();
    drawFishBody(SPECIES[0], W / 2 - 168, 90, 0.08, 1.35, state.time);
    ctx.fillStyle = "#fff6e8"; ctx.font = "700 28px Fredoka, sans-serif"; ctx.textAlign = "center";
    ctx.fillText("Aqua Bay Pier Mart", W / 2 + 18, 86);
    ctx.fillStyle = "#ffe27a"; ctx.font = "700 14px Nunito, sans-serif";
    ctx.fillText("Dive. Stock. Sell.", W / 2 + 18, 110);
    ...
    ctx.font = "700 13px Nunito, sans-serif";
      ctx.fillText("Aqua Bay · loop 64", W / 2, 178);
    ctx.restore();
    drawSkinPicker(W / 2, 252, 168, 176, 16);
    const pulse = 1 + Math.sin(state.time * 3) * 0.035;
    if (state.hasSave) {
      panelBtn("continue", W / 2 - 150, 452, 300, 56, "Continue", null, pulse);
      ...
      ctx.fillText("$" + (state.money | 0) + "  ·  " + nSp + " species unlocked", W / 2, 528);
      panelBtn("play", W / 2 - 150, 548, 300, 48, "New Game", "#3d6f7a");
    } else {
      panelBtn("play", W / 2 - 150, 460, 300, 56, "Play", null, pulse);
    }
  }
  function drawPause() {
    ...
    if (state.mode === "help") {
      ...
      ctx.fillText("Aqua Bay · loop 64", W / 2, 538);
      ...
      ctx.fillStyle = "#ffe27a"; ctx.font = "700 14px Nunito, sans-serif";
      ctx.fillText("Aqua Bay · loop 64", W / 2, 632);
    }
  }
