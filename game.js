...
(() => {
  ...
  const player = { x: 880, y: 920, vx: 0, vy: 0, facing: 0, bob: 0, catchProg: 0, target: null, radius: 16, goto: null, route: null, blockT: 0, walkPhase: 0, lean: 0, faceS: 1, pitch: 0, pendingAct: null, catchLatch: false, scoopLock: null, scoopTap: false, tillDwell: 0, holdGrace: 0, surfaceIntent: false };
  ...
  function resetSave() {
    ...
    state.hasSave = false;
    player.x = 880; player.y = 920; player.vx = 0; player.vy = 0; player.catchProg = 0; player.target = null; player.goto = null; player.route = null; player.blockT = 0; player.walkPhase = 0; player.lean = 0; player.faceS = 1; player.pitch = 0; player.pendingAct = null; player.catchLatch = false; player.scoopLock = null; player.scoopTap = false; player.tillDwell = 0; player.holdGrace = 0; player.surfaceIntent = false;
  ...
  function titleMenuLayout() {
    ...
    const cardGap = 24;
    const cardW = Math.min(330, Math.round((W - 88 - cardGap * 2) / 3));
    ...
    return {
      ...
      whoFont: Math.max(22, Math.round(H * 0.017)),
      whoY,
      portrait: true,
    };
  }
  function topCtrlBoxes() {
    if (portraitStage()) {
      ...
      let muteB = hudBox(pauseB.x - 6 - topBtn, 10, topBtn, topBtn);
      if (muteB.x < xCol - 2) {
        muteB = hudBox(xCol + rail - topBtn, 10 + topBtn + 6, topBtn, topBtn);
      }
      return { topBtn, pauseB, muteB, rail, xCol };
    }
  ...
  function railBarsReady() {
    if (portraitStage()) {
      return state.mode === "play" && (state.tutorial >= 5 || state.money >= 25 || state.scene === "ocean");
    }
  ...
  function actionBtnBox() {
    ...
    let y = H - sz.pad - h;
    if (portraitStage()) {
      x = clamp(viewWidth() / 2 - w / 2, 12, viewWidth() - w - 12);
      y = H - sz.pad - h;
  ...
  function upgradeBarBox() {
    if (portraitStage()) {
      ...
      const cw = strip.cw;
      const minCh = cssToStage(42, 62, 96);
      const x = strip.x;
      const y = strip.y + strip.h + 10;
      const room = Math.max(minCh * 4 + 24, H - y - 16);
      const fitCh = clamp(((room - 18) / 4) | 0, minCh, Math.round(H * 0.12));
      const h = 4 * (fitCh + 6) + 8;
  ...
  function playPointerDown(p, opts) {
    ...
    if (hit && String(hit).startsWith("up-") && portraitStage() && p.y > H * 0.42 && p.x < railGutterLeft()) {
      // A low-deck walk tap must never purchase. Rail taps (any height) still arm.
      hit = null;
    }
    if (hit) {
  ...
  function drawShop() {
    ...
    paintRailProp(OPEN_SIGN.x, OPEN_SIGN.y, 64, function () { drawHangingSign(OPEN_SIGN.x, OPEN_SIGN.y); });
  ...
  function drawOcean() {
    ...
    if ((canSurfaceNow() || bagIsFull()) && !scoopBlocksSurface()) {
      ...
      ctx.fillText(thumbCopy() ? "SURFACE" : "SURFACE  ·  SPACE or click", OCEAN.w / 2, 70);
      ctx.globalAlpha = 1;
    }
  ...
  function currentGoal() {
    ...
    if (!state.missionDone) {
      ...
      if (step === 0) return { text: thumbCopy() ? "Tap the glowing dock to walk" : "Walk to the glowing dock", target: { x: 880, y: 980 } };
  ...
  function drawSurfaceAssist() {
    ...
    const sz = actionBtnSize();
    const playW = viewWidth();
    const x = clamp(playW - 16 - w, 12, playW - w - 12);
  ...
  function ribbonLayout() {
    ...
    const leftPad = portraitStage() ? 12 : 210;
    const rightPad = portraitStage() ? (W - railGutterLeft() + 8) : (W - muteB.x + 16);
    const maxW = Math.max(280, Math.min(portraitStage() ? railGutterLeft() - 24 : 680, W - leftPad - rightPad));
    ...
    const playRight = portraitStage() ? railGutterLeft() : W;
  ...
  function drawHUD() {
    ...
    if (bagShown || bagGhosts.length) {
      ...
      const bw = Math.min(36 + pipN * 28, portraitStage() ? Math.max(80, railGutterLeft() - 24) : 340);
    ...
    if (state.scene === "shop" && nearBoat() && expeditionUnlocked()) {
      ...
      ctx.textAlign = "center";
      ctx.fillText(thumbCopy() ? "EXPEDITION  $35" : "SPACE · Expedition $35", eb.x + eb.w / 2, eb.y + eb.h / 2 + 8);
  ...
  function drawSkinPicker(cx, cy, cardW, cardH, gap, fonts) {
    ...
    for (let i = 0; i < SKIN_IDS.length; i++) {
      ...
      ctx.textAlign = "center";
      ctx.fillText(meta.name, x + cardW / 2, cy + cardH - Math.max(28, Math.round(cardH * 0.09)));
      ctx.fillStyle = selected ? "#ffe27a" : "#9ec8d0";
      ctx.font = "700 " + blurbPx + "px Nunito, sans-serif";
      ctx.fillText(meta.blurb, x + cardW / 2, cy + cardH - Math.max(12, Math.round(cardH * 0.04)));
  ...
  function drawTitle() {
    ...
    const boardY = ty + th * 0.10, boardH = th * 0.46;
    ctx.save();
    ...
    ctx.fillText("Aqua Bay · loop 79", W / 2, lay.stampY);
    ctx.restore();
    ...
    if (state.hasSave) {
      panelBtn("continue", W / 2 - lay.continueW / 2, lay.continueY, lay.continueW, lay.continueH, "Continue", null, pulse, lay.btnFont);
      ...
      panelBtn("play", W / 2 - lay.newW / 2, lay.newY, lay.newW, lay.newH, "New Game", "#3d6f7a", 1, lay.btnFont);
    } else {
      panelBtn("play", W / 2 - lay.continueW / 2, lay.playY, lay.continueW, lay.playH, "Play", null, pulse, lay.btnFont);
    }
    ctx.restore();
    menuYShift = 0;
  }
  function drawPause() {
    ...
    if (state.mode === "help") {
      ...
      const lines = [
        ...
        "On a phone, Speed / Bag / Catch / Cashier sit in the right rail — tap twice to buy",
      ...
      ctx.fillText("Aqua Bay · loop 79", W / 2, footY + (tall ? 28 : 20));
      ...
      ctx.fillText("Aqua Bay · loop 79", W / 2, footY + (tall ? 52 : 36));
    }
    ctx.restore();
    menuYShift = 0;
  }
