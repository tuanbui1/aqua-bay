...
(() => {
  ...
  // Loop 48 characters/fish + loop 53 walk/swim + loop 55 skyline + loop 56 cone/props + loop 57 pier/paddle + loop 58 plant + loop 59 clean blit/pier + loop 60 plant props + loop 61 NPC plate/shadow.
  ...
  function ensurePaint() {
    if (!paint.wood) {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        const g = c.getContext("2d");
        g.fillStyle = i % 2 ? "rgba(40, 18, 8, 0.10)" : "rgba(255, 220, 150, 0.08)";
        g.fillRect(0, 0, 240, 40);
        if (i % 3 === 0) {
          g.fillStyle = "rgba(20, 80, 72, 0.10)";
          g.fillRect(40, 8, 80, 24);
        }
        paint.planks.push(c);
      }
  ...
  function drawPierBoards(x, y, w, h, opts) {
    ensurePaint();
    const plank = (opts && opts.plank) || 30;
    ...
    for (let row = 0, yy = y; yy < y + h; row++, yy += plank - 0.35) {
      const ph = Math.min(plank + 0.4, y + h - yy);
      if (ph <= 0) break;
      const u = taper ? clamp((yy - y) / Math.max(1, h), 0, 1) : 1;
      const rowW = taper ? topW + (w - topW) * u : w;
      const rowX = taper ? x + (w - rowW) / 2 : x;
      // Long overlapping boards, unique source window + flip per cell.
      // No 1.8px vertical seam bar, no 168px joist stamp.
      let sx = rowX - hash2(row, 2) * 28;
      let n = 0;
      while (sx < rowX + rowW + 10) {
        const pw = 268 + hash2(row * 19 + n * 13, 1) * 210 + ((row + n * 5) % 5) * 18;
        const cell = plankCell(row * 29 + n * 17 + ((yy * 5) | 0) + n * 3);
        const overlap = 22;
        ctx.save();
        if (ART.ready && cell) {
          const cut = hash2(row + n, 6);
          const srcX = cell.x + cut * Math.max(0, cell.w * 0.42);
          const srcW = Math.max(48, cell.w - (srcX - cell.x));
          if (hash2(row, n + 11) > 0.5) {
            ctx.translate(sx + pw + overlap, yy);
            ctx.scale(-1, 1);
            ctx.drawImage(ART.img, srcX, cell.y, srcW, cell.h, 0, 0, pw + overlap, ph);
          } else {
            ctx.drawImage(ART.img, srcX, cell.y, srcW, cell.h, sx, yy, pw + overlap, ph);
          }
        } else if (paint.planks && paint.planks.length) {
          const pc = paint.planks[((row * 5 + n * 3) % paint.planks.length)];
          ctx.drawImage(pc, sx, yy, pw + overlap, ph);
        } else if (tile) {
          ctx.drawImage(tile, sx, yy, pw + overlap, ph);
        }
        ctx.restore();
        const wear = hash2(row, n + 3);
        if (wear > 0.78) {
          ctx.fillStyle = "rgba(255, 226, 160," + (0.03 + wear * 0.03) + ")";
          ctx.fillRect(sx + pw * 0.12, yy, pw * 0.4, ph * 0.32);
        } else if (wear < 0.16) {
          ctx.fillStyle = "rgba(40, 18, 8, 0.08)";
          ctx.fillRect(sx + pw * 0.22, yy, pw * 0.28, ph);
        }
        if (teal) {
      ...
      ctx.fillStyle = "rgba(24,12,6,0.12)";
      ctx.fillRect(rowX, yy + ph - 0.5, rowW, 0.7);
    }
    ctx.restore();
    ctx.save();
    if (taper) {
    ...
    ctx.fillStyle = "rgba(168, 108, 52, 0.10)";
    ctx.fillRect(x, y, w, h);
    ctx.restore();
    if (wetY != null) {
  ...
  function drawPlayer(x, y, pose) {
    ...
    if (skin === "dino") {
      ...
      dinoBody.addColorStop(0, "#54b45e");
      dinoBody.addColorStop(0.55, "#3d9a4a");
      dinoBody.addColorStop(1, "#2a6a34");
      ...
      ctx.fillStyle = "#7a8c9c"; ctx.fillRect(-15.2, -9, 3.4, 16);
  ...
  function drawDiveSign(cx, footY) {
    sitShadow(cx + 3, footY + 8, 48, 9, 0.4);
    if (blit("divepad", cx, footY, { scale: 0.68, noShadow: true })) return;
    ctx.save();
    ctx.translate(cx, footY);
    ctx.fillStyle = "rgba(16, 8, 4, 0.28)";
    ctx.beginPath(); ctx.ellipse(0, 6, 54, 8, 0, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = "#6a4220";
    roundRect(-62, -68, 124, 72, 10); ctx.fill();
    ctx.fillStyle = "#c4894a";
    roundRect(-54, -60, 108, 56, 7); ctx.fill();
    ctx.strokeStyle = "rgba(80, 40, 16, 0.28)";
    ctx.lineWidth = 1.1;
    for (let i = 0; i < 5; i++) {
      ctx.beginPath();
      ctx.moveTo(-50, -50 + i * 10);
      ctx.lineTo(50, -48 + i * 10);
      ctx.stroke();
    }
    ctx.strokeStyle = "#d4a060";
    ctx.lineWidth = 5.5;
    roundRect(-62, -68, 124, 72, 10); ctx.stroke();
    ctx.fillStyle = "#5a3614";
    ctx.beginPath(); ctx.arc(-48, -14, 4, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.arc(48, -14, 4, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = "#fff6e8";
    ctx.font = "800 22px Fredoka, sans-serif";
    ctx.textAlign = "center";
    ctx.fillText("DIVE", 0, -30);
    ctx.beginPath();
    ctx.moveTo(0, -16);
    ctx.lineTo(-16, 2);
    ctx.lineTo(-6, 2);
  ...
  function drawEastGallery() {
    ...
    drawPierBoards(1760, 360, 200, 530, { plank: 24, wetY: 890, teal: !!state.unlocked[1], taper: true, topW: 0.58 });
  ...
  function drawShop() {
    ...
    drawPierBoards(800, 360, 176, 530, { plank: 24, wetY: 890, teal: teal, taper: true, topW: 0.56 });
    ...
    // Wood DIVE sign on the left boards, beside the pad — not in the walk lane.
    const diveSign = { x: 556, y: 1008 };
    ...
    actors.push({
      y: diveSign.y,
      draw: function () {
        const diveA = worldBoxAlpha(diveSign.x - 70, diveSign.y - 88, 140, 104);
  ...
  function drawTitle() {
    ...
    drawBayWater(-20, waterY - 10, W + 40, H - waterY + 28, state.time, false);
    ctx.save();
    ...
    ctx.fillText("Aqua Bay · loop 61", W / 2, 178);
    ctx.restore();
  ...
  function drawPause() {
    ...
    if (state.mode === "help") {
      ...
      ctx.fillText("Aqua Bay · loop 61", W / 2, 538);
      ...
      ctx.fillStyle = "#ffe27a"; ctx.font = "700 14px Nunito, sans-serif";
      ctx.fillText("Aqua Bay · loop 61", W / 2, 632);
    }
  }
