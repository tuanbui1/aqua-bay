...
// loop 143 fish cruise in gentle S-curves, not straight lines
(() => {
  "use strict";
  ...
  function updateOceanFish(dt) {
    ...
    for (const f of oceanFish) {
      ...
      // loop 143 gentle swim S-curve — fish used to cruise in near-straight
  ...
  function drawOcean() {
    ...
    for (const f of list) {
      ...
      let dang = f.ang;
      // loop 143 — nose the fish into its actual swim velocity so the
  ...
  function drawTitle() {
    ...
    ctx.fillText("Aqua Bay · loop 143", W / 2, lay.stampY);
    ctx.restore();
  ...
  function drawPause() {
    ...
    if (state.mode === "help") {
      ...
      ctx.fillText("Aqua Bay · loop 143", W / 2, footY + (tall ? 28 : 20));
      ...
      ctx.fillText("Aqua Bay · loop 143", W / 2, footY + (tall ? 52 : 36));
    }
    ctx.restore();
    menuYShift = 0;
  }
