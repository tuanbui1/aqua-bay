...
(() => {
  ...
  function diveChipLegal() {
    ...
    // C112 — keep the DIVE chip while a dive-for hunt walk is live
    // (dock camera would otherwise hide it for ↑ SHOP).
    // C115 — keep the thumb DIVE live while DIVE FOR {species} is
    // legal on the empty bowl. Do not hide DIVE. That chip now
    // arms the same hunt. loop 115 dive chip arms the hunt.
    return diveActionLegal() || (diveWalkLegal() && (dockOffScreen() || plazaCameraReady() || diveForHuntIndex() >= 0 || diveForCueLegal()));
  }
  ...
  function applyHuntStockGoal() {
    ...
    // Do not auto-stock. loop 116 today hunt copy.
  ...
  function todayGoalLabel() {
    ...
    // "Take the boat" / "Unlock …" / "Serve 3…". A rolled boat
    // daily stays when there is no hunt and no empty unlocked
    // bowl to stock. New-game first DIVE does not force Stock
    // Seahorse. Do not auto-stock. loop 116 today hunt copy.
    applyHuntStockGoal();
    const want = huntStockIndex();
  ...
  function drawTitle() {
    ...
    ctx.fillText("Aqua Bay · loop 117", W / 2, lay.stampY);
    ctx.restore();
  ...
  function drawPause() {
    ...
    if (state.mode === "help") {
      ...
      ctx.fillText("Aqua Bay · loop 117", W / 2, footY + (tall ? 28 : 20));
      ...
      ctx.fillText("Aqua Bay · loop 117", W / 2, footY + (tall ? 52 : 36));
    }
    ctx.restore();
    menuYShift = 0;
  }
