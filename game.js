...
(() => {
  ...
  function diveChipLegal() {
    ...
    return diveActionLegal() || (diveWalkLegal() && (dockOffScreen() || plazaCameraReady() || diveForHuntIndex() >= 0));
  }
  ...
  function oceanEntrySpawn() {
    ...
    // A normal DIVE chip (no hunt) still drops in shallows.
    // loop 112 dive for the right band.
    if (state.expedition) {
  ...
  function drawTitle() {
    ...
    ctx.fillText("Aqua Bay · loop 114", W / 2, lay.stampY);
    ctx.restore();
  ...
  function drawPause() {
    ...
    if (state.mode === "help") {
      ...
      ctx.fillText("Aqua Bay · loop 114", W / 2, footY + (tall ? 28 : 20));
      ...
      ctx.fillText("Aqua Bay · loop 114", W / 2, footY + (tall ? 52 : 36));
    }
    ctx.restore();
    menuYShift = 0;
  }
