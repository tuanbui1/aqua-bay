...
(() => {
  ...
  function drawTitle() {
    ...
    ctx.fillText("Aqua Bay · loop 125", W / 2, lay.stampY);
    ctx.restore();
  ...
  function drawPause() {
    ...
    if (state.mode === "help") {
      ...
      ctx.fillText("Aqua Bay · loop 125", W / 2, footY + (tall ? 28 : 20));
      ...
      ctx.fillText("Aqua Bay · loop 125", W / 2, footY + (tall ? 52 : 36));
    }
    ctx.restore();
    menuYShift = 0;
  }
  ...
  function updateCam(dt) {
    ...
    // loop 125 one shore no water-walk boat tap only
    // No dock↔store bandJump. Follow the walker; cap stays on.
    ...
    if (psy > stageBot - padB) ny = player.y - (stageBot - padB - H / 2) / cam.z;
    // loop 125 one shore no water-walk boat tap only
    // One continuous shore camera. Follow the player from pier
    // (y~920–1000) up the aisle to the bowls. No band snap, no
    // fade, no smash-cut between dock and store.
