...
(() => {
  ...
  function clickWalkTarget(wx, wy) {
    if (state.scene === "shop") {
      ...
      if (westDockTapWanted(wx, wy) || onWestDockWalk(wx, wy)) return snapToShopWalk(wx, wy);
  ...
  function drawTitle() {
    ...
    ctx.fillText("Aqua Bay · loop 129", W / 2, lay.stampY);
    ctx.restore();
  ...
  function drawPause() {
    ...
    if (state.mode === "help") {
      ...
      ctx.fillText("Aqua Bay · loop 129", W / 2, footY + (tall ? 28 : 20));
      ...
      ctx.fillText("Aqua Bay · loop 129", W / 2, footY + (tall ? 52 : 36));
    }
    ctx.restore();
    menuYShift = 0;
  }
