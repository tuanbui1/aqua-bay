...
(() => {
  ...
  function clickOnBoat() {
    ...
    // Hull or the on-screen BOAT chip — not the whole nearBoat radius.
    // loop 127 — only the BOAT chip uses actionBtnBox. Never the DIVE
    // chip that shares that box.
  ...
  function diveChipLegal() {
    ...
    // loop 127 DIVE works on the dock — C126 one-shore cam.y~1000
    // is not plazaCameraReady. A step off the pad must not hide DIVE.
    // Do not require plazaCameraReady / dockOffScreen / nearDivePad.
  ...
  function tryAction(opts) {
    ...
    // loop 127 — skip if the shared actionBtnBox is the DIVE chip.
    ...
    if (state.scene === "shop" && (inDiveZone() || nearDivePad() || clickOnDiveHud() || (clickOnDiveChip() && nearDivePad()))) {
      ...
      // loop 127 — thumb HUD DIVE also counts (shares actionBtnBox with BOAT).
  ...
  function shopDockWalk() {
    ...
    // Boards include DIVE pad y=1008. Do not extend into the ocean.
  ...
  function snapToShopWalk(x, y, destHint) {
    ...
    // loop 125 one shore no water-walk boat tap only
    // Dock feet stay on painted wood. westRamp is water west of the
    // pier; eastShop is a plaza deck — not a dock snap onto ocean.
    ...
    function skipWater(rc) {
      if (rc.x + rc.w < dock.x && rc.y >= 650) return true;
  ...
  function drawPlayer(x, y, pose) {
    ...
    if (!skipAtlas && moving) {
      if (blitGait(skin, "walk", fi, x, plant, {
        scaleX: faceDrawX(faceS, squashX), scaleY: squashY + turnThin * 0.08,
        rot: lean * 0.16 + yawTwist, scale: blitSc, flat: true,
      })) return;
    } else if (
      (!skipCard && blit(skin + "_card", x, plant, {
        scaleX: faceDrawX(faceS), scaleY: 1 + turnThin * 0.06,
  ...
  function drawDiver(x, y, ang, t, skinId) {
    ...
    const drew = blitGait(skin, "swim", fi, x, y, {
      rot: tilt * flip + (1 - Math.abs(faceS)) * 0.16,
      scale: 0.58,
      scaleX: faceDrawX(faceS, 1 + Math.abs(kickWave) * 0.04),
  ...
  function drawTitle() {
    ...
    ctx.fillText("Aqua Bay · loop 127", W / 2, lay.stampY);
    ctx.restore();
  ...
  function drawPause() {
    ...
    if (state.mode === "help") {
      ...
      ctx.fillText("Aqua Bay · loop 127", W / 2, footY + (tall ? 28 : 20));
      ...
      ctx.fillText("Aqua Bay · loop 127", W / 2, footY + (tall ? 52 : 36));
    }
    ctx.restore();
    menuYShift = 0;
  }
