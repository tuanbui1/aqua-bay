...
// loop 146 pearls drop gems, a premium currency you can spend
(() => {
  "use strict";
  ...
  const state = {
    ...
    hiredCashier: false, cashierAcc: 0, diverLv: 0, diverAcc: 0, lastPlayed: 0, gems: 0,
  ...
  function persist() {
    try {
      ...
      localStorage.setItem(SAVE_KEY, JSON.stringify({
        ...
        hiredCashier: state.hiredCashier, diverLv: state.diverLv | 0, gems: state.gems | 0,
  ...
  function resetSave() {
    ...
    Object.assign(state, defaultSave(), { mode: "title", scene: "shop", fade: 0, coins: [], toasts: [],
      ...
      diverLv: 0, diverAcc: 0, lastPlayed: 0, gems: 0,
      sawReef: false, sawGoldGarden: false, sawKoiGate: false, sawTurtleMeadow: false,
  ...
  function onUI(id) {
    ...
    if (id === "gem-restock") { pearlRestock(); return; }
  ...
  // loop 146 — pearls & gems. Some fish are carrying a pearl; catching one
  // drops a gem (a rarer catch is likelier to). Gems are a premium currency
  // you spend to instantly restock the whole shop.
  const PEARL_CHANCE = 0.11;      // regular catch
  const PEARL_CHANCE_RARE = 0.4;  // shiny catch
  const RESTOCK_GEMS = 3;
  function maybePearl(f) {
    if (Math.random() >= (f && f.rare ? PEARL_CHANCE_RARE : PEARL_CHANCE)) return;
    ...
    for (let k = 0; k < 10; k++) {
      particles.push({
        x: player.x + rand(-10, 10), y: player.y + rand(-10, 10),
        vx: rand(-30, 30), vy: rand(-50, -14), life: rand(0.4, 0.8),
        r: rand(1.6, 3.2), col: pick(["#bfeaff", "#eaffff", "#9ef0ff", "#fff"]),
      });
    }
    persist();
  }
  function pearlRestock() {
    if ((state.gems | 0) < RESTOCK_GEMS) return nope({ card: "gem-restock" });
    let filled = 0;
    for (let s = 0; s < SPECIES.length; s++) {
      if (!speciesUnlocked(s)) continue;
      while ((state.stock[s] | 0) < DIVER_TANK_CAP) { diverStockOne(s); filled++; }
    }
    if (!filled) return nope({ card: "gem-restock" });
    state.gems -= RESTOCK_GEMS;
    sfx("unlock");
    toast("Pearl restock!  Every tank topped up.", "#9ef0ff");
    persist();
  }
  ...
  function updateFX(dt) {
    ...
    if (state.gemPop > 0) state.gemPop = Math.max(0, state.gemPop - dt * 1.8);
    tickDiveForCue(dt);
    if (state.comboPop) {
  ...
  function drawGems() {
    if ((state.gems | 0) <= 0) return;
    const muteB = topCtrlBoxes().muteB;
    const canRestock = state.scene === "shop" && (state.gems | 0) >= RESTOCK_GEMS;
    const txt = "\u25C6 " + (state.gems | 0) + (canRestock ? "   restock " + RESTOCK_GEMS : "");
    ctx.save();
    ctx.font = "800 15px Nunito, sans-serif";
    const gw = ctx.measureText(txt).width + 24;
    const gh = 32;
    const gx = muteB.x - 12 - gw, gy = muteB.y + (muteB.h - gh) / 2;
    const pop = 1 + (state.gemPop || 0) * 0.22;
    ctx.translate(gx + gw / 2, gy + gh / 2); ctx.scale(pop, pop); ctx.translate(-(gx + gw / 2), -(gy + gh / 2));
    pierChip(gx, gy, gw, gh, canRestock ? "rgba(24, 54, 62, 0.92)" : "rgba(16, 40, 50, 0.86)");
    if (canRestock) {
      ctx.strokeStyle = "rgba(158,240,255," + (0.4 + 0.3 * Math.sin(state.time * 5)) + ")";
      ctx.lineWidth = 2; roundRect(gx, gy, gw, gh, 10); ctx.stroke();
      btn("gem-restock", gx, gy, gw, gh);
    }
    ctx.fillStyle = "#bfeaff"; ctx.textAlign = "left";
    ctx.fillText(txt, gx + 12, gy + 21);
    ctx.restore();
  }
  ...
  function drawTitle() {
    ...
    ctx.fillText("Aqua Bay · loop 146", W / 2, lay.stampY);
    ctx.restore();
  ...
  function drawPause() {
    ...
    if (state.mode === "help") {
      ...
      ctx.fillText("Aqua Bay · loop 146", W / 2, footY + (tall ? 28 : 20));
      ...
      ctx.fillText("Aqua Bay · loop 146", W / 2, footY + (tall ? 52 : 36));
    }
    ctx.restore();
    menuYShift = 0;
  }
